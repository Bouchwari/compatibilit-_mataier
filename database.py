...
CATEGORIES = {
    "مكتبيات": [
        ...
        # خراطيش الطباعة — مدمجة مع المكتبيات
...
def init_db():
    ...
    # Migrate: merge خراطيش الطباعة into مكتبيات
    c.execute("UPDATE items    SET category='مكتبيات' WHERE category='خراطيش الطباعة'")
    c.execute("UPDATE movements SET category='مكتبيات' WHERE category='خراطيش الطباعة'")

    # Migrate: add new settings columns if missing
    _scols = [r[1] for r in c.execute("PRAGMA table_info(settings)").fetchall()]
    if "school_year" not in _scols:
        c.execute("ALTER TABLE settings ADD COLUMN school_year TEXT DEFAULT ''")
    if "amazigh_name" not in _scols:
        c.execute("ALTER TABLE settings ADD COLUMN amazigh_name TEXT DEFAULT ''")
...
def get_item_category(item_name):
    with get_conn() as conn:
        row = conn.execute("SELECT category FROM items WHERE name=?", (item_name,)).fetchone()
...
def update_movement(movement_id, date, quantity, beneficiary, notes):
    """Update an existing movement's editable fields."""
    with get_conn() as conn:
        conn.execute("""
            UPDATE movements SET date=?, quantity=?, beneficiary=?, notes=?
            WHERE id=?
        """, (date, quantity, beneficiary, notes, movement_id))
        conn.commit()
...
def get_settings():
    ...
    with get_conn() as conn:
        ...
        return {"school_name":"","academy":"","delegation":"",
                "address":"","logo_path":"","school_year":"","amazigh_name":""}
    ...
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO settings
                (id,school_name,academy,delegation,address,logo_path,school_year,amazigh_name)
            VALUES (1,?,?,?,?,?,?,?)
            ON CONFLICT(id) DO UPDATE SET
                ...
                amazigh_name=excluded.amazigh_name
