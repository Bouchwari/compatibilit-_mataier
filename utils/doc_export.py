...
def export_certificate_docx(cert_num, recipient, position, date, items):
    ...
    try:
        ...
        school_year  = _s.get("school_year",  "") or ""
        amazigh_name = _s.get("amazigh_name", "") or ""
    except Exception:
        school_name = academy = delegation = address = logo_path = ""
        school_year = amazigh_name = ""
    ...
    mapping = {
        ...
        ".../.../…….":                  date_ar,
        # School header lines — replace the hardcoded Almadoun text
        "الثانوية الإعدادية ألمدون":    school_name if school_name else "الثانوية الإعدادية ألمدون",
    }

    # Replace full academy/delegation lines if the user stored the full text,
    # otherwise do partial replacement of the region/city suffix only
    if academy:
        if "الأكاديمية الجهوية" in academy:
            # Full line stored — replace the entire default line
            mapping["الأكاديمية الجهوية للتربية والتكوين لجهة درعة تافيلالت"] = academy
        else:
            # Short value — replace just the region suffix
            mapping["درعة تافيلالت"] = academy
    if delegation:
        if "المديرية الإقليمية" in delegation:
            mapping["المديرية الإقليمية بـتنغير"] = delegation
            mapping["المديرية الإقليمية بتنغير"]  = delegation
        else:
            mapping["تنغير"] = delegation

    # School year line
    if school_year:
        mapping["السنة الدراسية: ....../......"] = f"السنة الدراسية: {school_year}"
        mapping["السنة الدراسية :"] = f"السنة الدراسية: {school_year}"

    # Amazigh / Tifinagh header name
    if amazigh_name:
        mapping["ⴰⵙⴳⵎⵉ ⵏ ⴰⵍⵎⴰⴷⵓⵏ"] = amazigh_name
...
def export_inventory_excel(dashboard_data):
    ...
    try:
        ...
        _s_ex = _db2.get_settings()
        _sn  = _s_ex.get("school_name","") or "المؤسسة"
        _acad = _s_ex.get("academy","") or ""
        _dele = _s_ex.get("delegation","") or ""
        _yr  = _s_ex.get("school_year","") or ""
        _addr = _s_ex.get("address","") or ""
        school = _sn + (" — " + _addr if _addr else "")
        sub_hdr = " | ".join(filter(None, [_acad, _dele, _yr]))
    except Exception:
        school = "كشف المخزون العام"
        sub_hdr = ""
    ...
    ws["A3"] = f"كشف المخزون العام — تاريخ الإصدار: {datetime.now().strftime('%Y-%m-%d')}"
    ws["A3"].font = Font(bold=True, size=11, color="FFFFFF")
    ws["A3"].fill = PatternFill("solid", fgColor="1A3A52")
...
def export_inventory_template_excel(all_items):
    ...
    try:
        ...
        _s_tmpl = _db3.get_settings()
        _sn_t = _s_tmpl.get("school_name","") or "المؤسسة"
        ws["A1"] = f"{_sn_t} — قالب الجرد السنوي"
    except Exception:
        ws["A1"] = "قالب الجرد السنوي"
